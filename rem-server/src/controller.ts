...
class Controller {
  ...
  public static getCategoryAndCountries = async (
    req: Request,
    res: Response
  ): Promise<void> => {
    ...
    try {
      const data = await Model.categoryAndCountries(
        path,
  ...
  public static getImage = (req: Request, res: Response): void => {
    try {
      ...
      const url =
        process.env.IMG_URL +
        'uploads/movies/' +
        encodeURIComponent(req.query.url as string) +
        '&w=192&q=75'
